...
exports.bmCancelVisit = exports.bmEditVisit = exports.bmArchiveTask = exports.bmEditTask = exports.bmRejectComplaint = exports.bmVisits = exports.bmComplaints = exports.bmBranches = exports.bmApprovals = exports.bmTasks = exports.bmAttendance = exports.bmDashboard = void 0;
 ...
 */
const bmDashboard = async (req, res) => {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ message: "Unauthorized" });
        if (user.role !== client_1.RoleId.branchManager && user.role !== client_1.RoleId.aa)
        ...
        const [branches, approvals, visits, notifications] = await Promise.all([
            ...
            prisma_1.default.notification.findMany({
                where: { scope: { has: client_1.RoleId.branchManager } },
                select: {
                    id: true,
                    title: true,
                    detail: true,
...
exports.bmDashboard = bmDashboard;
/**
 ...
 */
const bmAttendance = async (req, res) => {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ message: "Unauthorized" });
...
const bmApprovals = async (req, res) => {
    try {
        ...
        return res.status(200).json(approvals.map((a) => ({ ...a, requestedBy: a.requestedById })));
    }
    catch (error) {
...
exports.bmApprovals = bmApprovals;
/**
 * GET /api/bm/branches
 * Returns branches in scope with appliances and staff counts.
 */
const bmBranches = async (req, res) => {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ message: "Unauthorized" });
...
exports.bmComplaints = bmComplaints;
/**
 * GET /api/bm/visits
 * Returns visits for branches in BM scope.
 */
const bmVisits = async (req, res) => {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ message: "Unauthorized" });
 ...
 */
const bmRejectComplaint = async (req, res) => {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ message: "Unauthorized" });
        ...
        if (!scopedBranchIds.includes(existing.branchId))
            return res.status(403).json({ message: "Complaint is not within your scope" });
        if (existing.status === client_1.ComplaintStatus.RESOLVED) {
        ...
        const updated = await prisma_1.default.complaint.update({
            where: { id },
            data: { status: client_1.ComplaintStatus.RESOLVED, resolutionNotes: `Rejected by BM: ${reason || "No reason provided"}` },
            select: { id: true, complaintId: true, status: true, branchId: true, resolutionNotes: true, vendorRemarks: true, updatedAt: true },
        });
        (0, stats_1.recalcBranchStats)(existing.branchId).catch((err) => console.error("Failed to recalc branch stats after BM reject complaint:", err));
...
const bmEditTask = async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const scopedBranchIds = user.branchScope || [];
        const { title, notes, deadline, priority, assignedToId, checklistTotal, proofRequired, zone, schedule } = req.body;
        const existing = await prisma_1.default.check.findUnique({ where: { id }, select: { id: true, branchId: true, status: true } });
        if (!existing)
            return res.status(404).json({ message: "Check not found" });
        if (!scopedBranchIds.includes(existing.branchId))
            return res.status(403).json({ message: "Task is not within your scope" });
        if (existing.status === client_1.TaskStatus.Revoked)
            return res.status(409).json({ message: "Cannot edit an archived (revoked) task" });
        const updated = await prisma_1.default.check.update({
            where: { id },
            data: {
...
const bmArchiveTask = async (req, res) => {
    try {
        ...
        if (!scopedBranchIds.includes(existing.branchId))
            return res.status(403).json({ message: "Task is not within your scope" });
        if (existing.status === client_1.TaskStatus.Revoked)
            return res.status(409).json({ message: "Task is already archived" });
        const updated = await prisma_1.default.check.update({
            where: { id },
            data: { status: client_1.TaskStatus.Revoked },
...
const bmEditVisit = async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const scopedBranchIds = user.branchScope || [];
        const { scheduledAt, purpose, agenda } = req.body;
        const existing = await prisma_1.default.visit.findUnique({ where: { id }, select: { id: true, branchId: true, status: true } });
        if (!existing)
            return res.status(404).json({ message: "Visit not found" });
        if (!scopedBranchIds.includes(existing.branchId))
            return res.status(403).json({ message: "Visit is not within your scope" });
        if (existing.status === client_1.VisitStatus.Completed || existing.status === client_1.VisitStatus.Cancelled) {
        ...
        const updated = await prisma_1.default.visit.update({
            ...
            data: {
                ...
                status: client_1.VisitStatus.Scheduled, // reset to Scheduled on reschedule
            },
...
exports.bmEditVisit = bmEditVisit;
/**
 ...
 */
const bmCancelVisit = async (req, res) => {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ message: "Unauthorized" });
...
exports.bmCancelVisit = bmCancelVisit;
