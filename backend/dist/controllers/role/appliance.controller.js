...
exports.decommissionAppliance = exports.registerAppliance = void 0;
 ...
 */
const registerAppliance = async (req, res) => {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ message: "Unauthorized" });
        if (user.role !== client_1.RoleId.branchManager && user.role !== client_1.RoleId.rm) {
            return res.status(403).json({ message: "Forbidden: BM or RM only" });
        }
        const { branchId, name, category, brand, zone, model, serial, amcVendor, purchaseCost, purchaseDate, nextService, warranty, } = req.body;
        ...
        if (user.role === client_1.RoleId.branchManager) {
        ...
        const existing = await prisma_1.default.appliance.findUnique({ where: { serial }, select: { id: true } });
        if (existing)
            return res.status(409).json({ message: `An appliance with serial "${serial}" already exists` });
        const appliance = await prisma_1.default.appliance.create({
            data: {
                branchId,
                name,
                category,
...
exports.registerAppliance = registerAppliance;
/**
 ...
 */
const decommissionAppliance = async (req, res) => {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ message: "Unauthorized" });
        ...
        const existing = await prisma_1.default.appliance.findUnique({
            where: { id },
        ...
        if (existing.status === client_1.ApplianceStatus.Down) {
            return res.status(409).json({ message: "Appliance is already decommissioned (Down)" });
        }
        const updated = await prisma_1.default.appliance.update({
            where: { id },
            data: {
                status: client_1.ApplianceStatus.Down,
                healthScore: 0,
...
exports.decommissionAppliance = decommissionAppliance;
