...
import { RoleId, TaskStatus, ComplaintStatus, ApprovalStatus, VisitStatus, Priority } from "@prisma/client";
...
export const getDashboardMetrics = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (role === RoleId.lc) {
      ...
      // Fetch task stats
      const tasks = await prisma.task.findMany({ where: { branchId } });
      const pendingTasksCount = tasks.filter(t => t.status === TaskStatus.Pending).length;
      const activeTasksCount = tasks.filter(t => t.status === TaskStatus.Pending || t.status === TaskStatus.InProgress).length;
      ...
      const riskyAppliances = appliances.filter(
        a => a.status !== "Operational" || a.approvalStatus.includes("Pending")
      );

      // Fetch active tasks for action queue
      const activeTasks = await prisma.task.findMany({
        where: { branchId, status: { in: [TaskStatus.Pending, TaskStatus.InProgress] } },
      ...
      const pendingApprovals = await prisma.approval.findMany({
        where: { branchId: { in: branchScope }, status: ApprovalStatus.Pending },
      ...
      const incompleteVisitsCount = visits.filter(v => v.status !== VisitStatus.Completed && v.status !== VisitStatus.Cancelled).length;
    ...
    } else if (role === RoleId.rm) {
      // 3. Regional Manager Dashboard Metrics (RM views all branches)
      const branches = await prisma.branch.findMany();
      const approvals = await prisma.approval.findMany({ include: { requestedBy: { select: { name: true } } } });
      ...
      const pendingApprovalsCount = approvals.filter(a => a.status === ApprovalStatus.Pending).length;

      // Decision feed: high-cost complaints (>20k) and pending approvals at RM stage
      const decisionFeed: { text: string; priority: string }[] = [];
      complaints
        .filter(c => c.estimatedCost > 20000 && c.status !== ComplaintStatus.Resolved && c.status !== ComplaintStatus.Rejected)
        .forEach(c => {
          const br = branches.find(b => b.id === c.branchId);
          decisionFeed.push({
            text: `Approve ${c.estimatedCost > 40000 ? "capex" : "repair"} for ${br?.name || "branch"} — ${c.title}.`,
            priority: c.priority,
          });
        });

      approvals
        .filter(a => a.status === ApprovalStatus.Pending && a.stage === "RM")
        .forEach(a => {
          const br = branches.find(b => b.id === a.branchId);
          decisionFeed.push({
      ...
      approvals
        .filter(a => a.status === ApprovalStatus.Pending && a.priority === Priority.Critical)
        .forEach(a => {
          const br = branches.find(b => b.id === a.branchId);
      ...
      complaints
        .filter(c => c.status === ComplaintStatus.Escalated)
        .forEach(c => {
          const br = branches.find(b => b.id === c.branchId);
          watchlistItems.push(`${br?.name || "Branch"}: ${c.title} is escalated.`);
        });

      branches
        .filter(b => b.health < 80)
        .forEach(b => {
          watchlistItems.push(`${b.name} health score at ${b.health}% — review needed.`);
        });
